...
// CS 3312, spring 2019
// Studio 4
// YOUR NAMES: Chad Fry, Joshua Meek
...
document.addEventListener('DOMContentLoaded', function () {
   ...
   document.querySelector('#print-hailstone').addEventListener('click', function () {
      //Variable to use when calculating and outputting the hailstone
      var numToChange, length, outputString;
      outputString = ""; //Start with an empty string
      length = 1; // For anything to be executed, we need at LEAST one number.

      //Grab the value inside the box to get the hailstone number.
      numToChange = parseInt(document.querySelector('#starting-number').value, 10);
   ...
   document.querySelector('#draw-star-boxes').addEventListener('click', function () {
      //Variables for the dimensions of the box & for calculating distances
      var boxDimensions, row, col, boxOutput, nearestEdgeDistance,
      ...
      //Grab the value inside the box to get the size of the boxes
      boxDimensions = parseInt(document.querySelector('#size-of-outer-box').value, 10);
